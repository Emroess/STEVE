...
".config-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:12px}"
".config-item{}"
...
"<select id=\"presetLoad\" style=\"font:12px -apple-system,sans-serif;padding:6px;background:#fff;border:1px solid #d0d0d0;margin-right:4px\"></select>"
"<button onclick=\"loadAndApplyPreset()\">Apply</button></div>"
"</div>"
"<div class=\"config\">"
...
"</div>"
"<button onclick=\"saveConfig()\">Update Configuration</button>"
"</div>"
...
"function saveConfig(){const c={viscous:parseFloat(document.getElementById('viscous').value),"
...
"window.onload=()=>{loadPresets();fetch('/api/v1/config',{headers:{'X-API-Key':'" HTTP_API_KEY "'}}).then(r=>r.json()).then(j=>{"
...
"document.getElementById('damping').value=j.wall_damping});setInterval(update,100)}"
"</script></body></html>";
...
static bool handle_request(struct tcp_pcb *tpcb, http_state_t *hs) {
  ...
  if (strcmp(hs->method, "GET") == 0) {
    ...
    } else if (strcmp(hs->uri, "/api/v1/performance") == 0) {
      rest_api_handle_get_performance(tpcb);
