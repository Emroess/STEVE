...
{
  uart_write_string(NULL, "\r\n>>> low_level_init starting\r\n", 100);
  
  /* Derive MAC address from STM32 unique ID */
  uint32_t unique_id[3];
  board_get_unique_id(unique_id);
  uint8_t macaddress[6] = {
    0x02,  /* Locally administered */
    (uint8_t)(unique_id[0] & 0xFF),
    (uint8_t)((unique_id[0] >> 8) & 0xFF),
    (uint8_t)(unique_id[1] & 0xFF),
    (uint8_t)((unique_id[1] >> 8) & 0xFF),
    (uint8_t)(unique_id[2] & 0xFF)
  };
  ...
  uart_write_string(NULL, ">>> Initializing PHY...\r\n", 100);
  ...
  uart_write_string(NULL, ">>> PHY init OK, checking link state...\r\n", 100);
  ethernet_link_check_state(netif);
  uart_write_string(NULL, ">>> low_level_init complete\r\n", 100);
}
...
void HAL_ETH_MspInit(ETH_HandleTypeDef *heth)
{
  ...
  */
  ...
  /* Configure PB13 (RMII_TXD1 for Nucleo-H753ZI) */
...
void ethernet_link_check_state(struct netif *netif)
{
  ...
  uart_write_string(NULL, "\r\n*** ethernet_link_check_state called ***\r\n", 100);

  PHYLinkState = LAN8742_GetLinkState(&LAN8742);

  // Debug output for PHY link state
  char debug_msg[50];
  sprintf(debug_msg, "\r\nPHY Link State: %d\r\n", (int)PHYLinkState);
  uart_write_string(NULL, debug_msg, 50);
  
  sprintf(debug_msg, "netif_is_link_up: %d\r\n", netif_is_link_up(netif) ? 1 : 0);
  uart_write_string(NULL, debug_msg, 50);
  ...
  else if(!netif_is_link_up(netif) && (PHYLinkState > LAN8742_STATUS_LINK_DOWN))
  {
    ...
    if(linkchanged)
    {
      ...
      HAL_StatusTypeDef status = HAL_ETH_Start(&EthHandle);
      if (status == HAL_OK) {
        uart_write_string(NULL, "\r\nETH DMA started\r\n", 100);
      } else {
        uart_write_string(NULL, "\r\nETH DMA start FAILED\r\n", 100);
      }
...
void HAL_ETH_RxAllocateCallback(uint8_t **buff)
{
  ...
  else
  {
    ...
    *buff = NULL;
    uart_write_string(NULL, "RX buffer allocation failed\r\n", 100);
  }
}
...
void HAL_ETH_RxLinkCallback(void **pStart, void **pEnd, uint8_t *buff, uint16_t Length)
{
  ...
  *ppEnd  = p;
  ...
  /* Cache invalidation is NOT needed because MPU configures this region as non-cacheable.
   * WHY: Removing this prevents Bus Faults if Length is corrupted (e.g., from stale DMA descriptors).
   * The MPU fix ensures cache coherency without manual operations, eliminating the root cause
   * of firmware freezes ~30s after boot.
   */
}
